...
class RavenOSInstaller:
    ...
    def installation_process(self):
        try:
            ...
            self.log_message("Включение [multilib] репозитория...")
            # Раскомментируем [multilib] и строку Include под ним
            self.run_command("sed -i '/\\[multilib\\]/{s/^#//;n;s/^#//}' /mnt/etc/pacman.conf")
            # Обновляем базу данных пакетов в chroot
            ...
            packages = "steam wine-staging lutris gamemode mangohud vulkan-icd-loader lib32-vulkan-icd-loader"
            security_packages = "ufw apparmor firewalld"
            all_packages = f"{packages} {security_packages}"
            self.run_command(f"arch-chroot /mnt pacman -S --noconfirm {all_packages}")
            ...
            setup_commands = [
                ...
                # 1. AppArmor (ограничивает программы)
                "arch-chroot /mnt systemctl enable apparmor",

                # 2. UFW (брандмауэр) - включаем и задаем политики по умолчанию.
                "arch-chroot /mnt ufw default deny incoming",    # Запретить все входящие
                "arch-chroot /mnt ufw default allow outgoing",   # Разрешить все исходящие (для пользователя)
                "arch-chroot /mnt ufw enable",                   # Включить UFW

                # 3. Firewalld (для KDE) - отключаем, так как UFW проще
                "arch-chroot /mnt systemctl disable firewalld",

                # 4. Общие настройки ядра (hardening)
                # Блокируем загрузку неподписанных модулей ядра (защита от rootkits)
                "echo 'install usb-storage /bin/true' >> /mnt/etc/modprobe.d/blacklist.conf",
            ]
